...
const observer=new IntersectionObserver(entries=>entries.forEach(async entry=>{if(!entry.isIntersecting)return;const section=sections.find(s=>s.id===entry.target.id);if(!section||entry.target.dataset.loaded)return;entry.target.dataset.loaded='1';const cards=[...entry.target.querySelectorAll('.chart-card')];await Promise.all(cards.map(async(card,index)=>{const chart=section.charts[index];try{const rows=await loadDataset(chart.file||section.file);card.classList.remove('loading');const renderer=chart.renderer||section.renderer;if(renderer==='maturities')renderMaturityChart(card,rows,chart);else if(renderer==='yield-curves')renderYieldCurves(card,rows,chart);else renderChart(card,rows,chart)}catch(error){console.error(error);card.classList.remove('loading');card.innerHTML='<p class="empty">No se pudo cargar este conjunto de datos.</p>'}}));observer.unobserve(entry.target)}),{rootMargin:'400px'});
