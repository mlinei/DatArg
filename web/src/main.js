...
document.querySelector('#app').innerHTML=`<header class="topbar"><a class="brand" href="#inicio"><span>DA</span><b>DatArg</b></a><nav>${sections.map(s=>`<a href="#${s.id}">${s.title}</a>`).join('')}</nav><div class="live"><i></i>Datos públicos</div></header>
...
const navLinks=[...document.querySelectorAll('.topbar nav a')];const activeObserver=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting){navLinks.forEach(a=>a.classList.toggle('active',a.hash===`#${e.target.id}`))}}),{threshold:.25});document.querySelectorAll('.data-section').forEach(s=>activeObserver.observe(s));
